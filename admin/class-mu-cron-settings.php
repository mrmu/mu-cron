...
class Mu_Cron_Settings {
	...
	public function __construct( $plugin_name, $version ) {
		...
		include_once $partial_settings_dir . 'class-mu-cron-settings-factory.php';
		include_once $partial_settings_dir . 'class-mu-cron-settings-general.php';
		// include_once $partial_settings_dir . 'class-mu-cron-settings-others.php';

		$this->settings = array(
			'General' => Mu_Cron_Settings_Factory::get_instance('general'),
			// 'Others' => Mu_Cron_Settings_Factory::get_instance('others'),
		);
	}
	...
	public function sanitize( $input ) {
		$new_input = '';
		do_action('mu_cron_log', 'sanitize input: '. $input);
		...
		if (!is_array($new_input)) {
			do_action('mu_cron_log', 'sanitize output: '. $new_input);
		}
	...
	public function display_log_page() {
		...
		echo '<div class="wrap">';
		echo '<p>For adding data to log use the hook: <code>do_action( \'mu_cron_log\', $data );</code></p>';
